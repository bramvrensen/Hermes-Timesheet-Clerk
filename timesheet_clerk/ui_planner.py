...
def planner_prompt(monday: str, sunday: str, *, rebuild: bool) -> str:
    ...
    return (
        ...
        "For every work_item return exactly one mapping decision keyed by source_id with tier AUTO/PROPOSE/ASK, ignored flag when appropriate, why, why_not_auto, confidence and mapping_source. "
        "For bookable entries include booking_mode assignment/direct plus the selected assignment or direct_mapping. For ignored=true entries, booking_mode and booking targets may be omitted; Python normalizes the non-bookable placeholder state. Never invent a project/service/hour type for ignored travel, lunch or other excluded time. "
