...
def _upgrade_sync_prompt(prompt: str) -> str:
    ...
    prompt += (
        ...
        "continue with mapping in the same refresh run. Call timesheet_plan_active and select mapping targets deterministically: "
        ...
        "that were created by the coverage repair plus any already-existing PENDING or legacy-ingestion-sentinel entries. "
        "For all other entries, preserve all previously reviewed/mapped entries unchanged. Load timesheet_config_get, "
