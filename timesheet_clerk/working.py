...
Planner runs synchronize a week's working plan in-place. Human review changes
still create explicit revisions through PlanRepository.save_revision().
"""
...
def sync_week_plan(repo: PlanRepository, incoming: dict[str, Any]) -> dict[str, Any]:
    ...
    new_by_key = {_entry_key(row): row for row in candidate.get("entries") or []}
    ...
    for key, prior in old_by_key.items():
        ...
        row["source_missing_since"] = merged["source_sync_at"]
        output.append(row)

    output.sort(key=lambda row: (str(row.get("date") or ""), str(row.get("planned_start") or ""), str(row.get("entry_id") or "")))
    merged["entries"] = output
    merged["status"] = existing.get("status") if existing.get("status") in {"DRAFT", "IN_REVIEW"} else "DRAFT"

    validated = validate_plan(merged)
    path = repo._revision_path(validated["plan_id"], int(validated["revision"]))
    _atomic_write_json(path, validated)
    repo._write_active_pointer(validated)
...
def _entry_key(entry: dict[str, Any]) -> str:
    ...
    if ids:
        return "clockify:" + "|".join(ids)
    return "fingerprint:" + source_fingerprint(entry)
