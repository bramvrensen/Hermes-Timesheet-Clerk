...
def render_booking(repo: PlanRepository, plan_id: str) -> None:
    ...
    st.info("Guarded task, day and week booking. Generate / refresh owns mapping validity; booking performs duplicate preflight, POST and readback verification.")
    if receipts:
        st.caption(f"{len(receipts)} booking receipt(s) stored for this plan.")
    st.divider()
    render_week_booking(repo, plan, widget_scope="booking-tab")
