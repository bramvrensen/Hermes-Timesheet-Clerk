...
def _approval_content(snapshot: dict[str, Any]) -> dict[str, Any]:
    """Compare immutable approval content without timestamp-only metadata."""
...
class PlanRepository:
    ...
    def approve_snapshot(self, plan_id: str, revision: int) -> dict[str, Any]:
        ...
        if path.exists():
            ...
            if _approval_content(existing) != _approval_content(snapshot):
                raise StateConflict("approval snapshot already exists with different plan content")
            return existing
