...
def _render_blockers(blocked: list[dict[str, str]], scope: str) -> None:
    ...
    noun = "entry blocks" if len(blocked) == 1 else "entries block"
    with st.expander(f"Why is Book {scope} blocked? · {len(blocked)} {noun}", expanded=True):
...
def _render_confirmation(repo: PlanRepository, plan: dict[str, Any], entries: list[dict[str, Any]], scope: str, label: str) -> None:
    ...
    confirmed=st.checkbox("I checked these entries and want to book them",key=f"confirm-{key}");cols=st.columns(2)
    with cols[0]:
...
def render_day_booking(repo: PlanRepository, plan: dict[str, Any], day: str, entries: list[dict[str, Any]]) -> None:
    ready,blocked=_bookable_ids(entries);open_entries=[entry for entry in entries if not entry.get("ignored") and entry.get("review_state")!="skipped" and entry.get("reconciliation_state")!="BOOKED"];key=_state_key(plan,f"day-{day}");disabled=not open_entries or bool(blocked) or not ready
    if st.button("Book day",key=f"book-{day}",disabled=disabled,use_container_width=True,help="All open entries for this day must be reviewed and bookable."):st.session_state[key]={"entry_ids":ready}
    _render_blockers(blocked,"day");_render_confirmation(repo,plan,entries,f"day-{day}","Book day")


def render_week_booking(repo: PlanRepository, plan: dict[str, Any]) -> None:
    entries=list(plan.get("entries") or []);ready,blocked=_bookable_ids(entries);open_entries=[entry for entry in entries if not entry.get("ignored") and entry.get("review_state")!="skipped" and entry.get("reconciliation_state")!="BOOKED"];key=_state_key(plan,"week");disabled=not open_entries or bool(blocked) or not ready
    if st.button("Book week",type="primary",disabled=disabled,use_container_width=True,help="All open entries in the week must be reviewed and bookable."):st.session_state[key]={"entry_ids":ready}
    _render_blockers(blocked,"week");_render_confirmation(repo,plan,entries,"week","Book week")
