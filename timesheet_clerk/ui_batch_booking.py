...
def _render_confirmation(repo: PlanRepository, plan: dict[str, Any], scope: str, label: str) -> None:
    ...
    total_seconds = sum(
        int((row.get("entry") or {}).get("planned_duration_seconds") or 0)
...
def render_day_booking(repo: PlanRepository, plan: dict[str, Any], day: str, entries: list[dict[str, Any]]) -> None:
    ...
    open_entries = [
        ...
        and entry.get("reconciliation_state") != "BOOKED"
    ]
    key = _state_key(plan, f"day-{day}")
    # Never render a mysterious disabled button when there are open entries.
    # Clicking is the diagnostic entry point even when zero rows are currently eligible.
    if st.button(
        "Book day",
        key=f"book-{day}",
        disabled=not open_entries,
        use_container_width=True,
        help="Checks all open entries for this day and books the eligible ones. Ineligible entries stay in review.",
    ):
...
def render_week_booking(repo: PlanRepository, plan: dict[str, Any]) -> None:
    ...
    if st.button(
        ...
        disabled=not open_entries,
        use_container_width=True,
        help="Checks all open entries in the week and books the eligible ones. Ineligible entries stay in review.",
    ):
        st.session_state[key] = {"entry_ids": ready, "blocked": blocked}
        st.rerun()
    if open_entries:
