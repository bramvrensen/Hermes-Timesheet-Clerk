"""One-time runtime SKILL migration for the 0.6 decisions-only contract."""
...
_MARKER = "<!-- timesheet-clerk-runtime-guard:0.6.0 -->"
_GUARD = """
<!-- timesheet-clerk-runtime-guard:0.6.0 -->
## Mandatory Timesheet Clerk 0.6 planner contract
...
Python owns plan identity, Clockify source fidelity, durations, week metadata, coverage, revisions, merging, preservation of human review and persistence. Never use terminal, execute_code, filesystem or generic file tools for Timesheet Clerk state. Never delete/reset state as recovery. On any Clerk tool error, stop and report the exact error.
""".strip()
