...
_MARKER = "<!-- timesheet-clerk-runtime-guard:0.6.3 -->"
_GUARD = """
<!-- timesheet-clerk-runtime-guard:0.6.3 -->
## Mandatory Timesheet Clerk 0.6.3 planner contract
...
4. for ignored=true decisions, do not invent a booking target; booking_mode/assignment/direct_mapping may be omitted and Python normalizes the non-bookable state;
5. call `timesheet_mapping_apply` exactly once with the complete decisions array and the SAME `rebuild` flag.
...
Python owns plan identity, Clockify source fidelity, durations, week metadata, coverage, revisions, merging, preservation of human review, ignored-entry normalization, removed-source reconciliation and persistence. Never use terminal, execute_code, filesystem or generic file tools for Timesheet Clerk state. Never delete/reset state as recovery. On any Clerk tool error, stop and report the exact error.
""".strip()
