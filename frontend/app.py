...
repo = PlanRepository()
# Working revisions are only a short optimistic-lock/debug safety net. Immutable
# approvals and feedback live elsewhere, so two mutable revisions are enough.
...
def _format_hm(value: Any) -> str:
    ...
    try:
        ...
        return text.split("T", 1)[-1][:5] if "T" in text else text[:5]
...
def _target_line(entry: dict[str, Any], plan: dict[str, Any]) -> str:
    ...
    return " · ".join(str(v) for v in (mapping.get("customer_name"), mapping.get("project_name"), mapping.get("service_name"), mapping.get("hour_type_name")) if v) or "Direct mapping incomplete"
...
def _save_review(plan: dict[str, Any], entry: dict[str, Any], values: dict[str, Any], reason: str = "") -> None:
    ...
    st.session_state["review_flash"] = "Saved"
    # Full rerun reloads the just-saved revision immediately. Scroll restoration
    # below returns the browser to the edited card instead of jumping to top.
    st.rerun()
...
def _select_row(label: str, rows: list[dict[str, Any]], current: Any, key: str) -> dict[str, Any] | None:
    ...
    return st.selectbox(label, options, index=index, format_func=lambda row: "— select —" if row is None else _name(row), key=key)


def _direct_editor(plan: dict[str, Any], entry: dict[str, Any]) -> dict[str, Any]:
    mapping, ctx = deepcopy(entry.get("direct_mapping") or {}), plan.get("review_context") or {}
    ...
    for row in list(ctx.get("all_hour_types") or ctx.get("hour_types") or []):
        ...
        if rid and rid not in seen:
            seen.add(rid); hour_types.append(row)
    preferred = str(read_config().get("preferred_hour_type") or "").casefold()
    if preferred:
        hour_types.sort(key=lambda row: (0 if _name(row).casefold() == preferred else 1, _name(row).casefold()))
...
def _target_complete(reviewed: dict[str, Any]) -> bool:
    ...
    return all(str(mapping.get(k) or "").strip() for k in ("project_id", "service_id", "hour_type_id"))
...
def _editor(plan: dict[str, Any], entry: dict[str, Any]) -> None:
    if _status(entry) == "SKIP":
        if st.button("Restore entry", key=f"restore-{entry['entry_id']}"):
            _save_review(plan, entry, {"ignored": False}, "restored in review UI")
        return
    ...
    with cols[0]:
        ...
        if abs(edited-current) >= .001 and st.button("Save hours", key=f"save-hours-{entry['entry_id']}", use_container_width=True):
    ...
    for col, (delta, label) in zip(cols[1:6], [(-3600,"−1h"),(-1800,"−30"),(-900,"−15"),(900,"+15"),(1800,"+30")]):
        with col:
            if st.button(label, key=f"d-{delta}-{entry['entry_id']}", use_container_width=True):
                _save_review(plan, entry, {"planned_duration_seconds": max(0, int(entry.get("planned_duration_seconds") or 0)+delta)}, "duration adjusted")
    with cols[6]:
    ...
    if st.button("Skip entry", key=f"skip-{entry['entry_id']}"): _save_review(plan, entry, {"ignored": True}, "skipped in review UI")
    ...
    complete = _target_complete(reviewed)
    if not complete: st.caption("Choose a complete booking target before resolving this entry. Duration/skip edits can still be saved independently.")
    if st.button("Accept proposal" if needs_review else "Save changes", type="primary", key=f"save-{entry['entry_id']}", disabled=not complete):
...
def _entry_dialog(plan_id: str, entry_id: str, review_context: dict[str, Any]) -> None:
    plan = deepcopy(repo.get_latest(plan_id)); plan["review_context"] = review_context
    entry = next((r for r in plan.get("entries") or [] if r.get("entry_id")==entry_id), None)
...
def _select_plan() -> dict[str, Any]:
    active, catalog = repo.get_active(), _catalog(); ids = [r["plan_id"] for r in catalog]
    if st.session_state.get("selected_plan_id") not in ids: st.session_state["selected_plan_id"] = active["plan_id"]
    idx = ids.index(st.session_state["selected_plan_id"]); row = catalog[idx]; week = row.get("week") or {}; monday, sunday = str(week.get("monday") or ""), str(week.get("sunday") or "")
...
def _all_catalog_days() -> list[date]:
    days: list[date] = []
    for row in _catalog():
        start, end = date.fromisoformat(row["week"]["monday"]), date.fromisoformat(row["week"]["sunday"])
        cursor = start
        while cursor <= end:
...
def _shift_day(days: list[date], delta: int) -> None:
    current = st.session_state.get("timesheet_day_picker")
    if current not in days: current = days[0]
    idx = days.index(current); st.session_state["timesheet_day_picker"] = days[max(0,min(len(days)-1,idx+delta))]


def _day_navigator(stored: dict[str, Any]) -> date:
    days = _all_catalog_days(); monday = date.fromisoformat(stored["week"]["monday"])
    current = st.session_state.get("timesheet_day_picker")
    if current not in days: st.session_state["timesheet_day_picker"] = monday
    idx = days.index(st.session_state["timesheet_day_picker"])
    prev_col, picker_col, next_col = st.columns([1,4,1])
    with prev_col: st.button("◀ Day", disabled=idx<=0, use_container_width=True, on_click=_shift_day, args=(days,-1))
    with picker_col:
        selected = st.selectbox("Day", days, format_func=lambda d: d.strftime("%a %d-%m-%Y"), key="timesheet_day_picker", label_visibility="collapsed")
    ...
    if target and target["plan_id"] != stored["plan_id"]:
        st.session_state["selected_plan_id"] = target["plan_id"]; st.rerun()
    return selected
...
def _trigger_planner(plan: dict[str, Any]) -> None:
    ...
    launch_sync(root=repo.root, profile=profile, prompt=prompt); st.success(f"{profile} sync started. Status below updates automatically.")
...
def _sync_status_widget() -> None:
    ...
    st.info(f"Sync running via {status.get('profile','planner')}…") if status.get("status")=="running" else st.success("Sync finished. Click Refresh view to load the latest plan state.")


def _restore_scroll() -> None:
    entry_id = st.session_state.pop("scroll_to_entry", None)
    if not entry_id: return
    target = json.dumps(f"entry-{entry_id}")
    components.html(f"<script>setTimeout(()=>{{const e=window.parent.document.getElementById({target});if(e)e.scrollIntoView({{block:'center',behavior:'instant'}});}},80);</script>", height=0)
...
def _review_page(stored: dict[str, Any], plan: dict[str, Any]) -> None:
    ...
    if st.session_state.pop("review_flash", None): st.success("Saved")
