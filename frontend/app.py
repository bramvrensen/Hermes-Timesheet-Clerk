...
def _safe_rebuild_active_week(repo) -> None:
    ...
    week = active.get("week") or {}; monday, sunday = str(week.get("monday") or ""), str(week.get("sunday") or "")
    st.warning(f"Rebuild {monday} through {sunday} from live sources. The current plan is NOT deleted first; it remains active unless a complete validated replacement succeeds.")
    ...
    if st.button("Rebuild active week", type="primary", disabled=not confirmed, use_container_width=True):
        result = start_planner(repo.root, monday, sunday, rebuild=True); st.success(f"Safe rebuild started · run {result['run_id'][:8]}. Existing state remains available until replacement succeeds.")
...
def _generate_current_week_action(*, compact: bool = False) -> None:
    ...
    if st.button("Generate current week", type="primary", use_container_width=True, key=f"generate-current-{monday}"):
        result = start_planner(review.repo.root, monday, sunday, rebuild=False); st.success(f"Current-week generation started · run {result['run_id'][:8]}")


def _bootstrap_current_week() -> None:
    _generate_current_week_action(compact=False); _render_job_status()


def _direct_editor_scoped(plan: dict, entry: dict) -> dict:
    mapping = deepcopy(entry.get("direct_mapping") or {}); ctx = plan.get("review_context") or {}
    customer = review._select_row("Customer", ctx.get("customers") or [], mapping.get("customer_id") or "", f"c-{entry['entry_id']}"); customer_id = review._plain_id(customer)
    projects = [row for row in ctx.get("projects") or [] if not customer_id or review._plain_id(row.get("customer_id")) == customer_id]
    project = review._select_row("Project", projects, mapping.get("project_id") or "", f"p-{entry['entry_id']}"); project_id = review._plain_id(project)
    ...
    service = review._select_row("Task / service", services, mapping.get("service_id") or "", f"s-{entry['entry_id']}"); service_id = review._plain_id(service)
    hour_types, preserved_current = editor_hour_type_choices(ctx, service_id, current_service_id=mapping.get("service_id"), current_hour_type_id=mapping.get("hour_type_id"), current_hour_type_name=mapping.get("hour_type_name"))
...
def _editor_with_task_booking(plan: dict, entry: dict) -> None:
    review._timesheet_clerk_base_editor(plan, entry); render_task_booking(review.repo, plan, entry)
