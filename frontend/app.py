...
_THEME_COOKIE="timesheet_clerk_theme"; _THEME_CONTROLLER=CookieController(key="timesheet-clerk-theme-cookies")
...
def _plain_id(value:Any)->str:
    ...
    text=str(value or ""); return text.split(":",1)[1] if ":" in text else text

def _name(row:dict[str,Any]|None)->str:
    row=row or {}; return str(row.get("name") or row.get("title") or row.get("label") or row.get("id") or "")
...
def _time_range(entry):return f"{_format_hm(entry.get('planned_start'))}–{_format_hm(entry.get('planned_end'))}"
def _entry_label(entry):
    source=entry.get("source") or {}; desc=source.get("description") or entry.get("description") or "Untitled"; client=source.get("client") or {}; client_name=client.get("name") if isinstance(client,dict) else str(client or ""); return f"{client_name+' · ' if client_name else ''}{desc}"
...
def _persist_theme():_THEME_CONTROLLER.set(_THEME_COOKIE,st.session_state["timesheet_theme"],path="/",max_age=365*86400,secure=True,same_site="strict")
def _theme_css(theme):
    light="--bg:#fff;--surface:#f7f8fa;--card:#fff;--text:#1f2328;--muted:#667085;--border:#d8dee4;--auto:#f2fbf5;--propose:#fff9e8;--ask:#fff1f1;--booked:#f3f4f6;"; dark="--bg:#0e1117;--surface:#161b22;--card:#11161d;--text:#e6edf3;--muted:#9aa4b2;--border:#30363d;--auto:#10251a;--propose:#2b2412;--ask:#2b1719;--booked:#1b2028;"; root=dark if theme=="dark" else light; media="" if theme!="system" else f"@media(prefers-color-scheme:dark){{:root{{{dark}}}}}"
...
def _review_context(start,end):return load_review_context(start,end)
def _with_context(plan):result=deepcopy(plan); week=result["week"]; result["review_context"]=_review_context(week["monday"],week["sunday"]); return result

def _assignment_options(plan,entry):
    ...
    return rows
def _assignment_label(row):return str(row.get("display_label") or " · ".join(str(v) for v in (((row.get("customer") or {}).get("name") if isinstance(row.get("customer"),dict) else None),((row.get("project") or {}).get("name") if isinstance(row.get("project"),dict) else None),row.get("name")) if v) or _plain_id(row))
def _target_line(entry,plan):
    ...
    m=entry.get("direct_mapping") or {}; return " · ".join(str(v) for v in (m.get("customer_name"),m.get("project_name"),m.get("service_name"),m.get("hour_type_name")) if v) or "Direct mapping incomplete"
def _clean(plan):result=deepcopy(plan); result.pop("review_context",None); return result

def _save_review(plan,entry,values,reason=""):
    updated,proposal,reviewed=apply_review(_clean(plan),entry["entry_id"],values); saved=repo.save_revision(updated,expected_revision=int(plan["revision"])); repo.append_feedback(feedback_event(plan_id=saved["plan_id"],proposal=proposal,reviewed=reviewed,reason=reason)); st.rerun(scope="fragment")
def _set_skip(plan,entry,skip):_save_review(plan,entry,{"ignored":skip},"skipped in review UI" if skip else "restored in review UI")
def _status(entry):
...
def _pending(entries):return sum(1 for e in entries if not e.get("ignored") and (e.get("tier") or e.get("overall_tier")) in {"PROPOSE","ASK"} and e.get("review_state") not in {"confirmed","corrected","skipped"})
def _entry_summary(plan,entry):
...
def _select_row(label,rows,current,key):
    options=[None]+rows; index=next((i for i,r in enumerate(options) if r and _plain_id(r)==_plain_id(current)),0); return st.selectbox(label,options,index=index,format_func=lambda r:"— select —" if r is None else _name(r),key=key)

def _direct_editor(plan,entry):
    m=deepcopy(entry.get("direct_mapping") or {}); ctx=plan.get("review_context") or {}; customer=_select_row("Customer",ctx.get("customers") or [],m.get("customer_id") or "",f"c-{entry['entry_id']}"); cid=_plain_id(customer); projects=[r for r in ctx.get("projects") or [] if not cid or _plain_id(r.get("customer_id"))==cid]; project=_select_row("Project",projects,m.get("project_id") or "",f"p-{entry['entry_id']}"); pid=_plain_id(project); services=[r for r in ctx.get("services") or [] if not pid or not r.get("project_id") or _plain_id(r.get("project_id"))==pid]; service=_select_row("Task / service",services,m.get("service_id") or "",f"s-{entry['entry_id']}"); sid=_plain_id(service); types=[r for r in ctx.get("hour_types") or [] if sid and _plain_id(r.get("service_id"))==sid]
    if types and all(r.get("source")=="global_fallback" for r in types):st.caption("Simplicate exposes no service/hour-type relation here; showing global hour types for manual review.")
    ...
    typ=_select_row("Hour type",types,m.get("hour_type_id") or "",f"h-{entry['entry_id']}"); return {"customer_id":cid or None,"customer_name":_name(customer) if customer else None,"project_id":pid or None,"project_name":_name(project) if project else None,"service_id":sid or None,"service_name":_name(service) if service else None,"hour_type_id":_plain_id(typ) or None,"hour_type_name":_name(typ) if typ else None,"billable":bool(m.get("billable",True))}

def _editor(plan,entry):
    ...
    cols=st.columns([1.2,.7,.7,.7,.7,.7,.9]); current=_hours(entry.get("planned_duration_seconds")); original=_hours(entry.get("original_duration_seconds"))
    with cols[0]:
        edited=st.number_input("Hours",min_value=0.0,max_value=24.0,value=float(current),step=.25,format="%.2f",key=f"dur-{entry['entry_id']}-{plan['revision']}")
    ...
    mode=st.radio("Booking mode",["assignment","direct"],horizontal=True,index=0 if entry.get("booking_mode")=="assignment" else 1,key=f"mode-{entry['entry_id']}"); reviewed={"booking_mode":mode}
    if mode=="assignment":
...
def _select_plan():
    ...
    idx=ids.index(st.session_state["selected_plan_id"]); row=catalog[idx]; week=row.get("week") or {}; monday=str(week.get("monday") or ""); sunday=str(week.get("sunday") or ""); title,nav,theme,out=st.columns([4,3.5,2.4,1.1])
    with title:st.markdown("## ⏱️ Timesheet Clerk")
    with nav:
        p,l,n=st.columns([1,4,1])
        with p:
            if st.button("◀",disabled=idx<=0,use_container_width=True):st.session_state["selected_plan_id"]=ids[idx-1]; st.session_state.pop("timesheet_nav_date",None); st.rerun()
        with l:
        ...
        with n:
            if st.button("▶",disabled=idx>=len(ids)-1,use_container_width=True):st.session_state["selected_plan_id"]=ids[idx+1]; st.session_state.pop("timesheet_nav_date",None); st.rerun()
