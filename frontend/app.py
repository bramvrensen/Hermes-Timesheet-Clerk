...
def _source_line(entry: dict[str, Any]) -> str:
    ...
    if source.get("description"):
        bits.append(str(source["description"]))
    return " · ".join(bits)
...
def _init_theme() -> None:
    if "timesheet_theme" in st.session_state:
        return
    try:
        value = str(st.context.cookies.get(_THEME_COOKIE) or "system").lower()
    except Exception:
        value = "system"
...
def _persist_theme() -> None:
    _THEME_CONTROLLER.set(
        _THEME_COOKIE,
        st.session_state["timesheet_theme"],
        path="/",
        max_age=365 * 86400,
        secure=True,
        same_site="strict",
...
def _review_context(start: str, end: str) -> dict[str, Any]:
    return load_review_context(start, end)
...
def _assignment_options(plan: dict[str, Any], entry: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in candidates:
        ...
        if start and start > day or end and end < day:
            continue
        seen.add(iid)
        rows.append(item)
...
def _assignment_label(row: dict[str, Any]) -> str:
    customer = (row.get("customer") or {}).get("name") if isinstance(row.get("customer"), dict) else None
    project = (row.get("project") or {}).get("name") if isinstance(row.get("project"), dict) else None
...
def _target_line(entry: dict[str, Any], plan: dict[str, Any]) -> str:
    if entry.get("booking_mode") == "assignment":
        iid = _plain_id(entry.get("assignment"))
        row = next((r for r in _assignment_options(plan, entry) if _plain_id(r) == iid), None)
        return _assignment_label(row) if row else iid or "No assignment"
    mapping = entry.get("direct_mapping") or {}
    return " · ".join(
        str(v)
        for v in (
            mapping.get("customer_name"),
            mapping.get("project_name"),
            mapping.get("service_name"),
            mapping.get("hour_type_name"),
        )
        if v
    ) or "Direct mapping incomplete"
...
def _save_review(plan: dict[str, Any], entry: dict[str, Any], values: dict[str, Any], reason: str = "") -> None:
    ...
    repo.append_feedback(
        feedback_event(plan_id=saved["plan_id"], proposal=proposal, reviewed=reviewed, reason=reason)
    )
...
def _entry_summary(plan: dict[str, Any], entry: dict[str, Any]) -> None:
    ...
    st.markdown(
        f"<div class='tc-entry {css}'><div class='tc-row'><div class='tc-time'>{html.escape(_time_range(entry))}</div><div class='tc-hours'>{planned:.2f}h</div><div><div class='tc-title'>{html.escape(_entry_label(entry))}</div><div class='tc-sub'>Clockify: {html.escape(_source_line(entry))} · {clocked:.2f}h</div></div><div class='tc-target'>→ {html.escape(_target_line(entry, plan))}</div><div><span class='tc-badge {css}'>{status}</span></div></div></div>",
...
def _select_row(label: str, rows: list[dict[str, Any]], current: Any, key: str) -> dict[str, Any] | None:
    ...
    index = next((i for i, row in enumerate(options) if row and _plain_id(row) == _plain_id(current)), 0)
    return st.selectbox(
        label,
        options,
        index=index,
...
def _direct_editor(plan: dict[str, Any], entry: dict[str, Any]) -> dict[str, Any]:
    ...
    customer = _select_row("Customer", ctx.get("customers") or [], mapping.get("customer_id") or "", f"c-{entry['entry_id']}")
    ...
    project = _select_row("Project", projects, mapping.get("project_id") or "", f"p-{entry['entry_id']}")
    project_id = _plain_id(project)
    services = [row for row in ctx.get("services") or [] if not project_id or not row.get("project_id") or _plain_id(row.get("project_id")) == project_id]
    service = _select_row("Task / service", services, mapping.get("service_id") or "", f"s-{entry['entry_id']}")
    service_id = _plain_id(service)

    # Hour Type is intentionally tenant-global. It must not be filtered by
    # customer/project/service. The preferred Senior Consultant label is sorted
    # first when present but is never invented if Simplicate does not expose it.
    hour_types = list(ctx.get("all_hour_types") or ctx.get("hour_types") or [])
    ...
    if preferred:
        deduped.sort(key=lambda row: (0 if _name(row).casefold() == preferred else 1, _name(row).casefold()))
    hour_type = _select_row("Hour type", deduped, mapping.get("hour_type_id") or "", f"h-{entry['entry_id']}")
...
def _review_target_complete(reviewed: dict[str, Any]) -> bool:
    if reviewed.get("booking_mode") == "assignment":
        return bool(_plain_id(reviewed.get("assignment")))
    mapping = reviewed.get("direct_mapping") or {}
...
def _editor(plan: dict[str, Any], entry: dict[str, Any]) -> None:
    status = _status(entry)
    if status == "SKIP":
        if st.button("Restore entry", key=f"restore-{entry['entry_id']}"):
            _set_skip(plan, entry, False)
        return
    ...
    if entry.get("why_not_auto"):
        st.info(str(entry["why_not_auto"]))

    cols = st.columns([1.2, .7, .7, .7, .7, .7, .9])
    ...
    with cols[0]:
        edited = st.number_input(
            ...
            step=.25,
            format="%.2f",
            key=f"dur-{entry['entry_id']}-{plan['revision']}",
        )
    ...
    for col, (delta, label) in zip(cols[1:6], [(-3600, "−1h"), (-1800, "−30"), (-900, "−15"), (900, "+15"), (1800, "+30")]):
        with col:
            if st.button(label, key=f"d-{delta}-{entry['entry_id']}", use_container_width=True):
                _save_review(
                    plan,
                    entry,
                    {"planned_duration_seconds": max(0, int(entry.get("planned_duration_seconds") or 0) + delta)},
                    "duration adjusted",
                )
    with cols[6]:
        if st.button("Reset", key=f"reset-{entry['entry_id']}", disabled=abs(current - original) < .001, use_container_width=True):
            _save_review(plan, entry, {"planned_duration_seconds": int(entry.get("original_duration_seconds") or 0)}, "duration reset")

    if st.button("Skip entry", key=f"skip-{entry['entry_id']}"):
        _set_skip(plan, entry, True)
    ...
    reviewed: dict[str, Any] = {"booking_mode": mode}
    if mode == "assignment":
        ...
        reviewed["direct_mapping"] = _direct_editor(plan, entry)

    reason = st.text_input("Reason / learning note", key=f"reason-{entry['entry_id']}")
    complete = _review_target_complete(reviewed)
    if not complete:
        st.caption("Choose a complete booking target before resolving this entry. Duration/skip edits can still be saved independently.")
    if st.button(
        "Accept proposal" if needs_review else "Save changes",
        type="primary",
        key=f"save-{entry['entry_id']}",
...
def _render_day(plan: dict[str, Any], day: str, entries: list[dict[str, Any]]) -> None:
    clocked = sum(_hours(entry.get("original_duration_seconds")) for entry in entries)
    workable = sum(_hours(entry.get("planned_duration_seconds")) for entry in entries if not entry.get("ignored"))
    booked = sum(_hours(entry.get("planned_duration_seconds")) for entry in entries if entry.get("reconciliation_state") == "BOOKED")
    pending = _pending(entries)
    header, action = st.columns([5, 1])
    with header:
        st.markdown(
            f"<div class='tc-day-title'>{html.escape(day)}</div><div class='tc-day-meta'>Clocked {clocked:.2f}h · Workable {workable:.2f}h · Booked {booked:.2f}h · {'ready' if not pending else f'{pending} review'}</div>",
            unsafe_allow_html=True,
        )
    with action:
    ...
    for entry in entries:
        ...
        if st.button("Review / edit", key=f"edit-{entry['entry_id']}"):
            _entry_dialog(plan["plan_id"], entry["entry_id"], context)
...
def _select_plan() -> dict[str, Any]:
    ...
    idx = ids.index(st.session_state["selected_plan_id"])
    row = catalog[idx]
    ...
    with nav:
        ...
        with prev_col:
            if st.button("◀", disabled=idx <= 0, use_container_width=True):
                st.session_state["selected_plan_id"] = ids[idx - 1]
                st.session_state.pop("timesheet_nav_date", None)
                st.rerun()
        with label_col:
            try:
        ...
        with next_col:
            if st.button("▶", disabled=idx >= len(ids) - 1, use_container_width=True):
                st.session_state["selected_plan_id"] = ids[idx + 1]
                st.session_state.pop("timesheet_nav_date", None)
                st.rerun()
    with theme:
...
def _date_navigator(stored: dict[str, Any], view: str) -> date:
    catalog = _catalog()
    all_starts = [date.fromisoformat(row["week"]["monday"]) for row in catalog]
    all_ends = [date.fromisoformat(row["week"]["sunday"]) for row in catalog]
    monday = date.fromisoformat(stored["week"]["monday"])
    current = st.session_state.get("timesheet_nav_date")
    if not isinstance(current, date) or current < min(all_starts) or current > max(all_ends):
        st.session_state["timesheet_nav_date"] = monday
    selected = st.date_input(
        "Go to date",
        min_value=min(all_starts),
        max_value=max(all_ends),
        key="timesheet_nav_date",
    )
    target = next(
        (
            row
            for row in catalog
            if date.fromisoformat(row["week"]["monday"]) <= selected <= date.fromisoformat(row["week"]["sunday"])
        ),
        None,
    )
    if target and target["plan_id"] != stored["plan_id"]:
        st.session_state["selected_plan_id"] = target["plan_id"]
        st.rerun()
    if view == "day":
        st.session_state["timesheet_selected_day"] = selected.isoformat()
    return selected
...
def _trigger_planner(plan: dict[str, Any]) -> None:
    ...
    prompt = (
        ...
        "FIRST call timesheet_sync_probe with full ISO-8601 week timestamps. "
        "If it reports requires_rebaseline, establish a fresh Clockify source baseline while preserving human-reviewed booking state, then call timesheet_plan_sync. "
        "If has_changes is false, stop immediately and report only its deterministic summary. "
        "Do not load Simplicate, learning context or the full plan in the no-op path. "
        "If genuine changes exist, use only source_delta.new_entries and source_delta.changed_entries as Clockify source records, then read only the Simplicate/learning data needed for those deltas. "
        "Treat every normalized Clockify record as one immutable source bundle. Preserve canonical Clockify source snapshots in the sync payload. "
        "Then call timesheet_plan_sync. Present the deterministic summary returned by the tool; never recalculate counts or totals. "
        "Never use filesystem/file-search for Clerk state and never book hours."
    )
    launch_sync(root=repo.root, profile=profile, prompt=prompt)
    st.success(f"{profile} sync started. Status below updates automatically.")
...
def _sync_status_widget() -> None:
    ...
    if status.get("status") == "running":
        st.info(f"Sync running via {status.get('profile', 'planner')}…")
    else:
        st.success("Sync finished. Click Refresh view to load the latest plan state.")
...
def _review_page(stored: dict[str, Any], plan: dict[str, Any]) -> None:
    ...
    metrics[1].metric("Clocked", f"{clocked:.1f}h")
    metrics[2].metric("Workable", f"{workable:.1f}h")
    metrics[3].metric("Booked", f"{booked:.1f}h")
    metrics[4].metric("Open", f"{max(0, workable - booked):.1f}h")
    ...
    with c2:
        if st.button("↻ Generate / refresh plan", use_container_width=True):
            _trigger_planner(stored)
    with c3:
        if st.button("Refresh view", use_container_width=True):
            _review_context.clear()
            st.rerun()
    with c4:
        st.caption(f"{stored['plan_id']} · revision {stored['revision']} · {stored['status']} · planner: {read_config()['planner_profile']}")

    _sync_status_widget()
    selected_date = _date_navigator(stored, view)
    if abs(workable - target) >= .01:
        st.warning(f"Workable time is {workable - target:+.2f}h versus target.")
    if pending:
    ...
    with st.expander("⚙ Week settings", expanded=False):
        ...
        if st.button("Save target"):
            ...
            repo.save_revision(updated, expected_revision=int(stored["revision"]))
            st.rerun()
    ...
    if view == "day" and keys:
        ...
        if selected_day in days:
            ...
            st.info("No time entries for this date.")
    else:
        for day in keys:
    ...
    if view == "week":
        ...
        with approve:
            if st.button("Approve week", disabled=bool(pending), use_container_width=True):
                try:
                    snapshot = repo.approve_snapshot(stored["plan_id"], int(stored["revision"]))
...
def main() -> None:
    ...
    with review_tab:
        _review_page(stored, plan)
    with config_tab:
        render_config(repo, DEFAULT_SKILL)
    with skill_tab:
        render_skill(repo, DEFAULT_SKILL)
    with state_tab:
        render_state(repo, stored, plan.get("review_context") or {})
